...
class MainController:
    ...
    def __init__(self, stacked_widget, home_view, main_view, consulta_view, historico_view, model):
        ...
        # ===== Home =====
        self.home_view.btn_ir_cadastro.clicked.connect(
            lambda: self.stacked_widget.setCurrentIndex(1)
        )
        self.home_view.btn_ir_consulta.clicked.connect(
        ...
        v.table_resultados.itemDoubleClicked.connect(
            lambda *_: self._mostrar_historico_do_selecionado("consulta")
        )
        ...
        try:
            h.btn_ver_historico_h.clicked.connect(
                lambda: self._mostrar_historico_do_selecionado("historico")
            )
        except Exception:
            pass
        ...
        h.table_historico.itemDoubleClicked.connect(
            lambda *_: self._mostrar_historico_do_selecionado("historico")
        )
    ...
    def _clear_main_fields(self):
        ...
        try:
            ...
            pass
        try:
            mv.input_cliente.clear()
        except Exception:
            pass
        try:
            mv.input_modelo.clear()
        except Exception:
            pass
        try:
            mv.input_serial.clear()
        except Exception:
            pass
        try:
            mv.input_observacoes.clear()
        except Exception:
            pass
        try:
            mv.combo_tipo_servico.setCurrentIndex(0)
        except Exception:
            pass
        try:
            mv.combo_status.setCurrentIndex(0)
        except Exception:
            pass
        try:
            mv.combo_prioridade.setCurrentIndex(0)
        except Exception:
            pass
        try:
            mv.input_custo.setValue(0.0)
        except Exception:
            pass
        try:
            mv.input_garantia_meses.setValue(0)
        except Exception:
            pass
        try:
            mv.date_proxima.setDate(QDate.currentDate())
        except Exception:
            pass
    ...
    def _novo_servico_consulta(self):
        ...
        equip_label = label or ""
        dlg = ServiceDialog(self.stacked_widget, equip_label=equip_label)
        if dlg.exec_():
            vals = dlg.values()
    ...
    def _mostrar_historico_do_selecionado(self, origem="consulta"):
        t = (
            self.consulta_view.table_resultados
            if origem == "consulta"
            else self.historico_view.table_historico
        )
        equip_id, label = self._selected_equipment_from_table(t)
    ...
    def _historico_buscar(self):
        ...
        rows = self.model.consultar(
            ...
            prioridade=h.combo_prioridade_h.currentText()
            if hasattr(h, "combo_prioridade_h")
            else None,
