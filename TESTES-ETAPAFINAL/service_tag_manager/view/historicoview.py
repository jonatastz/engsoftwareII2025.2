...
class HistoricoView(QWidget):
    def __init__(self):
        ...
        self.input_termo_h = QLineEdit()
        self.input_termo_h.setPlaceholderText("tag, nome, cliente, modelo...")
        fl.addWidget(self.input_termo_h)

        fl.addWidget(QLabel("Status"))
        self.combo_status_h = QComboBox()
        self.combo_status_h.addItems(
            ["", "Recebido", "Em análise", "Em execução", "Pronto", "Entregue"]
        )
        fl.addWidget(self.combo_status_h)

        fl.addWidget(QLabel("Prioridade"))
        self.combo_prioridade_h = QComboBox()
        self.combo_prioridade_h.addItems(["", "Baixa", "Média", "Alta", "Urgente"])
        fl.addWidget(self.combo_prioridade_h)

        fl.addWidget(QLabel("Próx. Manut. de"))
        self.date_ini_h = QDateEdit()
        self.date_ini_h.setCalendarPopup(True)
        ...
        self.date_fim_h = QDateEdit()
        self.date_fim_h.setCalendarPopup(True)
        ...
        fl.addWidget(self.btn_buscar_h)
        fl.addWidget(self.btn_limpar_h)
        ...
        bar.addWidget(self.lbl_count_h)
        bar.addStretch(1)
        for b in [
            self.btn_ver_historico_h,
            self.btn_novo_servico_h,
            self.btn_editar_h,
            self.btn_excluir_h,
            self.btn_voltar_historico,
        ]:
            bar.addWidget(b)
